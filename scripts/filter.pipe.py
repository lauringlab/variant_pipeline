...
parser = argparse.ArgumentParser(description='This is a wrapper set up to run secondary analysis on deepSNV varaint calls. It filters the calls based on variables set in filter.pipe.groovy and then identifies nonsynonymous mutations based on the plasmid control concensus sequence.')

parser.add_argument(action='store',dest='input_dir',nargs='+',help='Diretory that contains the varaint csv files')
#parser.add_argument(action='store',dest='coding',nargs='+',help='The name of the reference fasta file for aligning the sequences')
parser.add_argument(action='store',dest='ref',nargs='+',help='The name of the reference fasta file that is a used to call variants against')
parser.add_argument(action='store',dest='OR',nargs='+',help = 'Fasta file of the open reading frames')
...
print "Processing variants from " + input_dir


# add variables to the bpipe config file to pass them to the pipeline
with open('./filter.config.groovy','w') as config:
...
if test==False:
    command= bpipe_command + " run -n 4 -r " + script_dir+ "/filter.pipe.groovy " + input_dir + "/Variants/*.sum.csv " + input_dir +"/deepSNV/*.fa"
else:
    command = bpipe_command + " test -n 4 " + script_dir +  "/filter.pipe.groovy " + input_dir +"/Variants/*.sum.csv " + input_dir +"/deepSNV/*.fa"
