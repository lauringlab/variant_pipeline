...
parser = argparse.ArgumentParser(description='This script takes in a fasta test fasta file and trims it to match the regions found a reference fasta file.\n I initially used it to trim whole genomes to just the coding regions, but I suppose it could have other uses. Currently it relies on MUSCLE. The segment names in the sample file must match those in the reference.')
...
parser.add_argument('-concat',action='store_true',dest='concat',default=False,
                    help='Optional tag. Use if you want to deconcatenate a concensus sequence into different segments')
...
def trim(aligned_headers_seqs):
    """trims excess 5' and 3' regions from sample seq.
    ...
    start_excess=start_excess# +1 # slicing is inclusive on this end ## Not sure this is true
    end=True
    i=len(ref_seq)-1
    end_excess=i
    #print(i)
    ...
    print "final segment is %d bases long" % (end_excess-start_excess)
...
def main(): # The positions will be given as base 0 and adjusted to match the convention (base 1) in the funciton
    ...
    if args.concat==False:
        ...
        for seqname in samp_seqname:
            ...
            alignments=Align([ref_seq, sample_seq], alignerpath,args.megaopen)
    ...
    elif args.concat==True: # # If the starting is concatenated then we need to cycle through the reference chromosomes but reuse the sample one each time
        for seq in ref:
            ref_seqname.append(seq.id)
        # make alignments
        print("Making %s alignments..." % prog)
        ...
        off_5=[]
        off_3=[]
        for i in range(0,len(align_ref)):
            print "Trimming %s" % align_ref[i].id
            trimmed_out=trim([align_ref[i],align_samp[i]])
            trimmed.append(trimmed_out[0])
            trimmed[i].id=align_ref[i].id # fixing id to match what is in the reference
            segs.append(align_ref[i].id)
            off_5.append(trimmed_out[1])
            off_3.append(trimmed_out[2])
