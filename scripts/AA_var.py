...
parser.add_argument('OR', metavar='OR', nargs='+',
                    help='The fasta file of open reading frames')
...
for ref in reference:
    ...
    for code in coding:
        ...
        if seg in OR:
            ...
            fixed=variants.loc[(variants["freq.var"]>0.95) & (variants["chr"]==seg)] # Assuming above 95% is fixed
            # Mutate the fixed variants in the reference sequence since all the genomes will have these.
            fixed_ref=mutate(ref,fixed)
            # Align
            fix_ref_coding=Align([fixed_ref,code],"/Users/jt/muscle3.8.31/")
            ref_coding=Align([ref,code],"/Users/jt/muscle3.8.31/")
            ...
            if fixed_ref_trimmed.seq.count("-")>0:
                raise ValueError, "Gaps were introduced in during alignment to the open reading frame - check sequence for insertions"
            ref_trans=ref_trimmed.seq.translate() # write a check this has a stop codon
            ...
            # Get reference AA ""
                variants.loc[i,"Ref_AA"].append(catch(lambda : ref_trans[int(variants.loc[i,"AA_pos"][-1])-1])) # the minus 1 is to convert to python numbering
