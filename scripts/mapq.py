                    ...
                    if pileupcolumn.pos==py_pos: #If the position is the one we want
                        for pileupread in pileupcolumn.pileups: #Now for each read in that column
                            if  not pileupread.is_del and not pileupread.is_refskip:
                                ...
                                if called_phred>29 and called_base==var: # change this if you change the phred cut off in deepSNV. deepSNV only looks a phred>30. and we only want those calles that match the variant.
